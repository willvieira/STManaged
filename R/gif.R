...
make_gif <- function(lands, steps = NULL, years = NULL, fps = 6, gifName = NULL, rangeLimit = FALSE, occup = 0.5)
{
  ...
  for(i in lds) {
    ...
    assign(figName, image_graph(width = 800, height = 500, res = 60, pointsize = 20, clip = TRUE)) # create obj to save `plot_landscape`; clip = FALSE speeds up a lot the process
    if(rangeLimit == TRUE) {
      rangeLim <- range_limit(lands[[i]], occup = occup)
    }else rangeLim <- NULL
