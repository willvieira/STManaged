...
plot_occ_time <- function(land) {

  # data frame
  df <- setNames(data.frame(matrix(1:4, nrow = 1)), c('B', 'T', 'M', 'R'))

  # land
  for(i in 1:land[['steps']]) {
    ld <- sapply(unlist(land[[i]][[1]]), convertState)
    df[i, ] <- as.vector(table(ld)/length(ld))
  }

  # plot
  col <- c("darkcyan","orange","palegreen3","black")
  main <- paste0('Plant = ', land[['manag']][[1]], '; Harv = ', land[['manag']][[2]],
                 '; Thin = ', land[['manag']][[3]], '; Enrich = ', land[['manag']][[4]],
