...
#' @param steps vector, time steps to be plotted. Remember to set \code{mfrow} before the function in case steps has more than one step
#' @param spar numeric, value between 0 and 1 to sooth the state occupancy
#' @export
#' @examples
#' \dontrun{
#' par(mfrorw = c(1, 2))
#' plot_occupancy(lands, steps = c(1, 100), spar = 0.2)
#' }

plot_occupancy <- function(lands, steps, spar)
{
  # define coordinates
  nCol <- lands[['nCol']]
  nRow <- lands[['nRow']]
  ...
  # checks
  if(!all(steps %in% 0:lands[['steps']])) stop('steps vector must be included in 1:steps')

  for(i in steps) {

    # get step name
    stepName <- paste0('land_T', i)
